...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[Npgbotlist]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.on('ready',async () => {
    console.log("[Npgbotlist]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'Npgbotlist | '+bots.length+' bots' }, status: "online" });
});
